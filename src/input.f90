...
subroutine input_get(ndim,lb,ub,delx,om,pot,coord,npoints,Vc)
  implicit none
  real(kind=8), dimension(:), allocatable, intent(inout) :: lb,ub,delx,om
  ...
  !precalcualted potential
    ...
    do i=0,ndim-1
      read(100,*) j,coord(i),npoints(i),lb(i),ub(i),delx(i),om(i)
    end do
    close(unit=100)
  end if
  ...
  write(*,'(1x,A9,4x,A4,4x,A6,6x,A11,5x,A11,9x,A3,14x,A4)') "dimension","type","points","lower bound","upper bound","Δx","ω"    
  do i=0,ndim-1
    write(*,'(1x,6x,I3,6x,I2,7x,I3,5x,ES12.5,4x,ES12.5,4x,ES12.5,4x,ES12.5)') i+1,coord(i),npoints(i),lb(i),ub(i),delx(i),om(i)
  end do
  write(*,*) 
  call input_save(ndim,npoints,coord,lb,ub,delx,om)
...
subroutine input_save(ndim,npoints,coord,lb,ub,delx,om)
  implicit none
  real(kind=8), dimension(0:), intent(in) :: lb,ub,delx,om
  ...
  write(100,*) ndim
  do i=0,ndim-1
    write(100,*) i,coord(i),npoints(i),lb(i),ub(i),delx(i),om(i)
  end do
  close(unit=100)
